...
**ตรวจจับนัดหมายอัตโนมัติ:** ตรวจพร้อมกับตอนสรุปแชท (ทั้ง `/สรุป` และสรุปรายวัน) ในการเรียก Gemini ครั้งเดียวกัน ไม่ได้เช็คแยกทุกข้อความ (เดิมทำแบบนั้นแล้วกินโควตา Gemini หมดเร็วมาก) — ถ้าเจอนัดหมายในช่วงที่สรุป จะประกาศในกลุ่มพร้อมวันเวลาที่ตรวจพบ แล้วรอให้พิมพ์ `/ยืนยันนัด` เพื่อบันทึกจริง (ไม่บันทึกอัตโนมัติ กันกรณี AI เข้าใจผิด)

> ⚠️ **ข้อจำกัดสำคัญ:** Gemini API แผนฟรีจำกัดแค่ **20 requests/วัน** ต่อโปรเจกต์ (ไม่ใช่ต่อนาที) ทุกฟีเจอร์ที่เรียก Gemini (`/สรุป`, `/ถาม`, สรุปรายวัน, ตรวจนัดหมาย) ใช้โควตาร่วมกันหมด ถ้าใช้งานบ่อยในวันเดียวอาจเจอ error "สรุปไม่สำเร็จ" เพราะโควตาหมด ต้องรอถึงวันถัดไปหรืออัปเกรดเป็นแผนเสียเงินที่ https://ai.google.dev/gemini-api/docs/rate-limits
